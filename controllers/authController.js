...
exports.forgotPassword = catchAsync(async (req, res, next) => {
    ...
    if (!user) return next(new appError('There is no user with provided email', 404));
...
exports.resetPassword = catchAsync(async (req, res, next) => {
    ...
    const user = await User.findOne({ passwordResetToken: hashedToken, passwordExpires: { $gt: Date.now() } });
